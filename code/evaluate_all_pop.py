...
movie_genre = pd.read_csv("/data/ml-1m-split/movie_pop_count_5.csv")
        ...
        for topk in topk_list:
            for i in range(len(test_data)):
                for k in range(topk):
                    ...
                    pop_genre = movie_genre["Pop"].iloc[rank[i][k].item()]
                    topk_count[topk]["%d" % pop_genre] += 1



        f = open('./log/movie_all_pop_fairness_on_ml_1m.json', 'w') 
